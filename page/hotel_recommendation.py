    ...
    if task_option == 'Topic Modeling':
        ...
        if((location_option != 'Select city...')):
            ...
            if st.sidebar.button("Train"):
                hotel = HotelRecommendation()
                st.markdown('<p style="color:lightgreen; font-size: 30px;"> 1. Training Auto Encoder to create embedding layer</p>', unsafe_allow_html=True)
                ae_embeddings = hotel.train_autoencoder(source_dict[location_option], encoding1_dim, encoding2_dim, latent_dim, epochs)
                st.markdown('<p style="color:lightgreen; font-size: 30px;"> 2. KMeans clustering</p>', unsafe_allow_html=True)
